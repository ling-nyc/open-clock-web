...
function formatErrorMessage(error: any): string {
  ...
  if (typeof error === 'string') {
    try {
      ...
      if (errObj.instancePath && errObj.schemaPath && errObj.instancePath.includes('dateTimeFormat')) {
        // Extract layer number if present
        const match = /layers","(\d+)"/.exec(JSON.stringify(errObj.instancePath));
...
const ClocksOrError: FunctionComponent<{
  ...
  if (errors.length > 0) {
    return (
      <div className="error">
        <div style={{fontWeight: 600, marginBottom: 4}}>Error loading clock file:</div>
        <ul style={{textAlign: 'left', margin: 0, paddingLeft: 18}}>
          {errors.map((e, i) => (
            <li key={i}>{formatErrorMessage(e)}</li>
          ))}
        </ul>
        <div style={{marginTop: 8, fontSize: '0.95em', color: '#666'}}>Check your file for typos or unsupported values. Only certain dateTimeFormat values are allowed.</div>
      </div>
    );
...
const App: FunctionComponent = () => {
  ...
  const handleFullscreen = () => {
    ...
    if (node && node.firstElementChild && (node.firstElementChild as any).requestFullscreen) {
      (node.firstElementChild as any).requestFullscreen({ navigationUI: 'show' });
    }
  };
    ...
    <TimeProvider>
      <AssetWarningProvider>
        <div className="app-container">
          ...
          <EntryArea jsons={jsons} setJsons={setJsons} onFullscreen={handleFullscreen} />
